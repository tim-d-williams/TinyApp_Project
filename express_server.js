...
app.post("/login", (req, res) => {
  for (let key in users) {
    if (users[key].email === req.body.email) {
        ...
        res.redirect('/urls');
      } res.status(403).send('<h1>Login or password is incorrect</h1>');
    }
  } res.status(403).send('<h1>Email address doesn\'t exist</h1>');
});
  ...
  if (user_id) {
    if (urlDatabase[urlId]) {
     if (urlDatabase[urlId].user_id === user_id) {
        ...
        res.send('<h1>Only URL owner can access this page</h1>');
      }
    } else {
      res.status(404).send('<h1>This URL does not exist</h1>');
    }
...
app.post('/urls/:id/delete', (req, res) => {
  ...
  if (user_id) {
    if (urlDatabase[urlToDelete].user_id === user_id) {
      ...
      res.send('<h1>Only URL owner can delete</h1>');
      }
    } else {
    res.send('<h1>Please login to access this page</h1>');
    }
